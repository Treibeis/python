...
def modify(cata, ref, dm = 10, t = 0, factor = 1.0):
        edge = ref[1]
        his = ref[0]
        raw = []
        count = 0
        ...
        for k in range(num0):
                if cata[2][k]>=ref[1][0]:
                        raw.append((cata[0][k],cata[1][k],cata[2][k],cata[3][k],cata[4][k]))
                        count += 1
        ...
        for i in range(nub):
                ...
                if t==0:
                        sub.reverse()
                if sub!=[]:
                        for k in range(int(factor*his[i])):
                                out[0].append(sub[k][1][0])
                                out[1].append(sub[k][1][1])
                                out[2].append(sub[k][1][2])
...
def convert1(lreal, zr = [0.2,0.6], fo = 'out1.txt', label = 'R-P', d_z = D_Z_L, H_z = H_Z_L, ob = (0.0, 0.0, 0.0), box = (2592.0, 2592.0, 2592.0), sky = [[0.0,360.0],[-90.0,90.0]], ko = 0):
