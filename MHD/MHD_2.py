...
def locus():
	...
	print('locus_s1(lam, t, x0, xm, l, d = {}, n, ini = [1.01, 1.01], k = 0, f = 1, con = 4, mindx, m0)\n')
...
class begin:
	...
	def to(self, x = 0.01, n = 10000, num = 1000, c = 0):
		...
		if c==1:
			...
			self.v1 = d2['v'][0]
		return d2
	def find_void(self, lim, t = 10, cri = 1.0/10**4, n = 10000, num = 1000):
		self.number = self.number + 1
		d1 = {'l':self.lam, 'x1':self.x1, 'x2':float((lim[1]+lim[0])/2.0), 'a':self.a1, 'v':self.v1, 'n':n}
		d2 = RungeKutta(d1, num)
		delta = (lim[1]+lim[0])/2.0 - d2['v-a'][0]
		count = 0
		a = lim[0]
		b = lim[1]
		while (abs(delta)>cri)and(count<t):
			if delta>0:
				b = (b+a)/2.0
			else:
				a = (b+a)/2.0
			c = (b+a)/2.0
			d1 = {'l':self.lam, 'x1':self.x1, 'x2':c, 'a':self.a1, 'v':self.v1, 'n':n}
			d2 = RungeKutta(d1, num)
			delta = c-d2['v-a'][0]
			count += 1
		if count==t:
...
def shock2(x, l, lam, l0 = [10.1,10.1], con = 4, up = 1.0/10**6):
...
def shock2_(x, l, lam, l0 = [0.1, 0.1], con = 4, up = 1.0/10**6):
